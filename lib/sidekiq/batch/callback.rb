...
module Sidekiq
  class Batch
    class Callback
      ...
      def death(status, queue)
        # Run any death callbacks
        if status.callbacks[DEATH]
          status.callbacks[DEATH].each do |hash|
            hash.each_pair do |target, options|
              execute_callback(status, DEATH, target, options)
            end
          end
        end

        # nothing further to do, this callback is informative only.
      end
      ...
      def success(status, queue)
        ...
        if pb
          ...
          if pending == 0 && successes == kids && cbcomp && cbcomp.to_i > 1 && !cbsucc && needs_success?(pb)
            enqueue_callback(queue, ['success', pb, q || queue])
          end
        end
      ...
      def complete(status, queue)
        ...
        if pb
          ...
          if complete == children && pending == fails && !cbcomp && needs_complete?(pb)
            enqueue_callback(queue, ['complete', pb, q || queue])
          end
        end
        ...
        if pending.to_i == 0 && children == bsucc && needs_success?(bid)
          enqueue_callback(queue, ['success', bid, q || queue])
        end
      end
