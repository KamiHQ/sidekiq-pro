    ...
    class Client
      ...
      def call(worker_class, msg, queue, redis_pool)
        ...
        if batch
          msg['bid'] = batch.bid
          result = yield
          batch.register(msg['jid']) if result
          result
        else
          yield
        end
    ...
    class Server
      ...
      def add_success(bid, jid, queue)
        ...
        if pending.to_i == failures.to_i && children == bcomp && !cbcomp && needs_complete?(bid)
          enqueue_callback(q || queue, ['complete', bid, q || queue])
        end
        ...
        if pending.to_i == 0 && children == bsucc && cbcomp && cbcomp.to_i > 1 && !cbsucc && needs_success?(bid)
          enqueue_callback(q || queue, ['success', bid, q || queue])
        end
      end
