...
module Sidekiq::Pro
  ...
  class SuperFetch
    ...
    def self.check_for_orphans
      ...
      Sidekiq.redis do |conn|
        ...
        conn.scan_each(:match => "queue:sq|*", :count => 100) do |que|
          ...
          if !conn.sismember("super_processes", id)
            qs << name
            loop do
              job = conn.rpoplpush(que, "queue:#{name}")
              break unless job
      ...
      if orphans > 0
        Sidekiq::Pro.metrics.increment("jobs.recovered.fetch", by: orphans)
    ...
    class Retriever
      ...
      def start(options)
        ...
        @algo = (options[:strict] && @queues.length == @queues.uniq.length) ? method(:strict) : method(:weighted)
      ...
      def cleanup_the_dead
        ...
        Sidekiq.redis do |conn|
          conn.sscan_each("super_processes") do |x|
            ...
            qs.each do |priv|
              (_, _, q) = priv.split("|")
              loop do
                job = conn.rpoplpush(priv, "queue:#{q}")
                break unless job
                Sidekiq.logger.warn { "SuperFetch recovered job: #{job}" }
                count += 1
              end
        ...
        if count > 0
          Sidekiq::Pro.metrics.increment("jobs.recovered.fetch", by: count)
          Sidekiq.logger.warn("SuperFetch: recovered #{count} jobs")
        end
        count
      end
