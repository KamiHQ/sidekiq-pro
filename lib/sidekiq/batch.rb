...
module Sidekiq
  ...
  class Batch
    ...
    LINGER = ONE_DAY
    VALID_EVENTS = %w(complete success death)
    ...
    def initialize(bid=nil)
      ...
      if bid
        ...
        @created_at = props['created_at'].to_f
      ...
      begin
        ...
        if mutable?
          ...
          data = ['created_at', created_at,
                  'callbacks', Sidekiq.dump_json(callbacks),
        ...
        redis(bid) do |conn|
          conn.multi do
            ...
            if @new
              conn.zremrangebyscore('batches', '-inf', Time.now.to_f)
            ...
            if myparent
              conn.hincrby("b-#{myparent}", "kids", 1)
    ...
    def increment_batch_jobs_to_redis(conn, jids)
      ...
      conn.hincrby(@key, "pending", jids.size)
      conn.hincrby(@key, "total", jids.size)
